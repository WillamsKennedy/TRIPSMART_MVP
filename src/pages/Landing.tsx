...
const featuredDestinations = [
  ...
  },
  {
    ...
    image: "🎶",
    tag: "Forró",
    color: "from-accent to-secondary/70",
          ...
          <div className="flex items-center gap-2">
            {user ? (
              <>
                ...
                <Button
                  onClick={() => goToPlanner()}
                  className="gradient-pe border-0 rounded-full px-5 h-9 text-sm font-bold gap-1.5 text-primary-foreground"
                >
              ...
              <Button
                ...
                className="gradient-pe border-0 rounded-full px-6 font-bold gap-2 text-primary-foreground"
              >
      ...
      <section className="relative overflow-hidden py-24 md:py-36 px-6 bg-hero-pattern">
        ...
        >
          ...
          <p className="text-lg md:text-xl text-muted-foreground mt-6 max-w-2xl mx-auto leading-relaxed">
            Roteiros personalizados com IA para cidades pernambucanas. Pontos turísticos, trilhas, praias, hospedagem e
            rotas — tudo em um só lugar.
          </p>
          <div className="flex flex-wrap justify-center gap-4 mt-10">
            <Button
              onClick={() => goToPlanner()}
              className="gradient-pe border-0 rounded-full px-10 h-14 text-lg font-bold gap-2 shadow-lg text-primary-foreground hover:opacity-90 transition-opacity"
            >
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {featuredDestinations.map((dest, i) => (
              ...
              >
                <div
                  className={`h-36 md:h-44 bg-gradient-to-br ${dest.color} flex items-center justify-center text-6xl md:text-7xl`}
                >
                  {dest.image}
                </div>
                <div className="p-5">
                ...
                <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                  <span className="text-xs font-bold px-3 py-1.5 rounded-full gradient-pe text-primary-foreground">
                    Planejar →
                  </span>
                </div>
              </motion.button>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                className={`text-xs font-bold px-3 py-2 rounded-full transition-all ${!cityFilter ? "gradient-pe text-primary-foreground" : "bg-background border border-border text-muted-foreground hover:border-primary/40"}`}
              >
                Todas
              </button>
              {pernambucoCities.map((c) => (
                <button
                  ...
                  className={`text-xs font-bold px-3 py-2 rounded-full transition-all ${cityFilter === c.id ? "gradient-pe text-primary-foreground" : "bg-background border border-border text-muted-foreground hover:border-primary/40"}`}
                >
                  {c.name}
                </button>
              ))}
            ...
            {allSpots.slice(0, 12).map(({ spot, cityId, cityName }, i) => (
              ...
              >
                {spot.imageUrl ? (
                  ...
                  <div className="w-full h-40 bg-gradient-to-br from-primary/15 to-accent/15 flex items-center justify-center text-5xl">
                    {spot.imageEmoji}
                  </div>
                )}
                <div className="p-4">
          ...
          {allSpots.length > 12 && (
            <p className="text-center text-sm text-muted-foreground mt-6">
              Mostrando 12 de {allSpots.length} atividades · Use os filtros acima
            </p>
          )}
        </div>
      </section>
      ...
      {!user && (
        <section className="py-24 px-6">
          <div className="max-w-4xl mx-auto text-center gradient-pe rounded-3xl p-14 md:p-20">
            <h2 className="text-3xl md:text-5xl font-black text-primary-foreground tracking-display leading-tight">
              Pronto para explorar Pernambuco?
            </h2>
            ...
            <Button
              onClick={() => navigate("/auth")}
              className="mt-10 bg-background text-foreground hover:bg-background/90 rounded-full px-10 h-14 text-lg font-bold gap-2"
            >
