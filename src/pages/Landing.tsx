...
import { MapPin, Sun, Compass, Star, ArrowRight, LogOut, History, Search, Filter, Users, Navigation } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {featuredDestinations.map((dest, i) => (
              ...
              >
                <div className={`h-36 md:h-44 bg-gradient-to-br ${dest.color} flex items-center justify-center text-6xl md:text-7xl`}>
                  {dest.image}
                </div>
                <div className="p-5">
      ...
      <section className="py-20 px-6 bg-card/50 border-y border-border">
        <div className="max-w-7xl mx-auto">
          ...
          {allSpots.length > 12 && (
            <p className="text-center text-sm text-muted-foreground mt-6">Mostrando 12 de {allSpots.length} atividades · Use os filtros acima</p>
          )}
        </div>
      </section>
      ...
      {!user && (
        <section className="py-24 px-6">
          <div className="max-w-4xl mx-auto text-center gradient-pe rounded-3xl p-14 md:p-20">
            ...
            <Button onClick={() => navigate('/auth')} className="mt-10 bg-background text-foreground hover:bg-background/90 rounded-full px-10 h-14 text-lg font-bold gap-2">Começar agora <ArrowRight size={20} /></Button>
          </div>
        </section>
      )}
